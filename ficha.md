# <<👌Mi primera ficha de Markdown>>

## 👤 Presentación  
Hola, soy **Federico>** y esta es mi primera ficha en Markdown.  

### 🎯 Objetivo de esta ficha  
Aprender a escribir documentos claros y bonitos usando Markdown.  
- **Negrita**: **Esta lloviendo**  
- *Cursiva:* *Esta lloviendo*  
- ~~Tachado:~~ ~~Ya no esta lloviendo~~  
- Resaltado (si tu plataforma lo soporta): ==Esta lloviendo==  
- Código en línea: `print("Hola Markdown")`  

#### ✅ Listas útiles  

##### Lista con viñetas  
- Markdown es rápido  
- Markdown es legible  
- Markdown se usa en GitHub y documentación  

###### Lista numerada (pasos)  
1. Escribir el texto  
2. Agregar formato  
3. Revisar en vista previa  
4. Publicar

###### ✅ Checklist  
- [x] Agregué títulos  
- [x] Agregué links  
- [x] Agregué una imagen  
- [x] Agregué emojis  
- [x] Revisé que se vea bonito  

###### 🔗 Enlaces  
  
Mi página favorita para aprender:  
- [YouTube](https://www.youtube.com/)  

Un video recomendado:  
- [La Segunda Guerra Mundial en 17 minutos](https://www.youtube.com/watch?v=AYQ8hT8cVTE)  

- [Ir a listas útiles](#-listas-útiles)  
 
###### 🖼️ Imagen  

![Imagen de ejemplo](https://i.pinimg.com/736x/03/cf/18/03cf187d88bb4f4fded6b85d73bc02d9.jpg)  

![Mi imagen](img/https://i.pinimg.com/736x/03/cf/18/03cf187d88bb4f4fded6b85d73bc02d9.jpg)  

###### 🌈 Emojis con intención  

Hoy me siento: 😎😊👌  
Mis herramientas favoritas: ⚽🏈⛳🛠️🔭  

## 🧩 Bloques para un buen diseño

> **Tip:** si tu documento se entiende sin explicarlo en voz alta, está bien escrito.

---

### Código en bloque (Python)
```python
def saludar(nombre):
    return f"Hola, {nombre}!"

print(saludar("mundo"))

## 📌 Tabla rápida  

| Tema | ¿Qué aprendí? |  
|------|----------------|  
| Títulos | Aprendi a decorar titulos |  
| Links | Aprendi a agregar links |  
| Imágenes | Aprendi a adjuntar imagenes |  